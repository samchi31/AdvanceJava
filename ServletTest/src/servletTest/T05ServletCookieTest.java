...
public class T05ServletCookieTest extends HttpServlet {
	...
	/*
	 * 쿠키정보를 다루기 위한 예제 (웹서버와 브라우저는 애플리케이션에서 사용중인 필요한 값을 쿠키를 이용하여 상태정보를 유지함)
	 * 
	 * 1. 구성요소 - 이름 - 값 - 유효시간(초) - 도메인 : ex) www.somehost.com, .somehost.com - 경로 :
	 * 쿠키를 공유할 기준경로 (도메인 이후부분으로 디렉토리 수준) => 지정하지 않으면 실행한 URL의 경로부분으로 지정됨
	 * 
	 * 2. 동작방식 - 쿠키생성단계 : 생성한 쿠키를 응답데이터의 헤더에 저장하여 웹브라우저에 저장 - 쿠키저장단계 : 브라우저는 응답데이터에
	 * 포함된 쿠키를 쿠키저장소에 보관한다 - 쿠키전송단계 : 브라우저는 저장한 쿠키를 요청이 있을 때마다 웹서버에 전송 (삭제되기 전까지...)
	 * 웹서버는 브라우저가 전송한 쿠키를 사용해 필요한 작업을 수행
	 * 
	 */
	@Override
...
//		readCookieExam(req, resp);		// 쿠키 정보읽기 예제
	...
	private void deleteCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		 ...
		 * 사용 중인 쿠키 삭제 방법
		 * 
		 * 1. 사용중인 쿠키정보를 이용하여 쿠키객체를 생성한다 2. 쿠키객체의 최대지속시간을 0으로 설정한다 3. 설정한 쿠키객체를 응답헤더에
		 * 추가하여 전송한다
		 */
		...
		if (cookies != null) {
			...
			System.out.println("<h2>쿠키 정보가 없습니다</h2>");
		}
	...
	private void setCookieExam(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		// 쿠키 생성하기
		// 사용불가문자(공백, []()=,:/?@:;) => RFC2965
		// 쿠키값은 사용불가 문자를 제외한 나머지 출력가능한 아스키 문자만 사용가능
		...
		// 쿠키 최대 지속시간 설정하기(초 단위)
		// 지정하지 않으면 웹브라우저 종료 시 쿠키정보는 삭제된다
		...
		out.println("<html><head><title>" + title + "</title></head><body>" + "<h1 align=\"center\"" + title + "</h1>"
				+ "<ul>" + "<li><b>ID<b>:" + req.getParameter("userId") + "</li>" + "<li><b>이름</b>:"
